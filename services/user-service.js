...
export const createUser = async (userData) => {
  ...
  const query = `
    ...
    RETURNING *
  `;
  const { rows } = await pool.query(query, [user_id, username, first_name, last_name, avatar]);
  return rows[0];
};
...
export const updateUser = async (user_id, updates) => {
  ...
  const query = `UPDATE users SET ${setClause} WHERE user_id = $${keys.length + 1} RETURNING *`;
...
export const saveRefreshToken = async (user_id, refreshToken) => {
  ...
  try {
    await pool.query('UPDATE users SET refresh_token = $1 WHERE user_id = $2', [refreshToken, user_id]);
  ...
  export const getRefreshToken = async (user_id) => {
    const { rows } = await pool.query('SELECT refresh_token FROM users WHERE user_id = $1', [user_id]);
