  ...
  try {
    ...
    if ((!ad_id && !post_id) || !images || !Array.isArray(images)) {
      return res
        .status(400)
        .json({
          error: "Необходимо указать ad_id или post_id и массив images",
        });
    }

    // Проверяем существование объявления или поста
    if (ad_id) {
      const adCheck = await pool.query("SELECT id FROM ads WHERE id = $1", [
        ad_id,
      ]);
    ...
    for (const image of images) {
      ...
      if (is_main) {
        if (ad_id) {
          ...
          await pool.query(
            "UPDATE ad_images SET is_main = false WHERE post_id = $1",
            [post_id]
          );
        }
      }
...
routerAdImages.get("/api/ad-images", async (req, res) => {
  try {
    ...
    if (ad_id) {
      ...
      const result = await pool.query(
        "SELECT * FROM ad_images WHERE post_id = $1 ORDER BY is_main DESC, created_at ASC",
        [post_id]
      );
      rows = result.rows;
    } else {
      return res
        .status(400)
        .json({ error: "Необходимо указать ad_id или post_id" });
    }
...
routerAdImages.delete("/api/ad-images", async (req, res) => {
  try {
    ...
    if (!ids || !Array.isArray(ids)) {
      return res
        .status(400)
        .json({ error: "Необходимо указать массив ids для удаления" });
    }
...
routerAdImages.post("/api/ad-images/set-main/:image_id", async (req, res) => {
  ...
  if (!image_id || (!ad_id && !post_id)) {
    return res
      .status(400)
      .json({ error: "Необходимо указать image_id и ad_id или post_id" });
  }
  try {
    ...
    if (ad_id) {
      ...
      await pool.query(
        "UPDATE ad_images SET is_main = true WHERE id = $1 AND ad_id = $2",
        [image_id, ad_id]
      );
    } else if (post_id) {
      await pool.query(
      ...
      await pool.query(
        "UPDATE ad_images SET is_main = true WHERE id = $1 AND post_id = $2",
        [image_id, post_id]
      );
    }
...
// Обновить URL изображений на публичный путь
routerAdImages.post("/api/ad-images/update-urls", async (req, res) => {
  try {
    const { rows } = await pool.query(`
      UPDATE ad_images 
      SET image_url = REPLACE(image_url, '/api/upload/uploads/', '/uploads/')
      WHERE image_url LIKE '%/api/upload/uploads/%'
      RETURNING id, image_url
    `);
    res.json({ success: true, updated: rows.length, images: rows });
  } catch (error) {
