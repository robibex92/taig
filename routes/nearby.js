...
routerNearby.get('/api/nearby', async (req, res) => {
  try {
    ...
    const filtered = rows.map(({ house, entrance, status, created_at, info, ...rest }) => ({
      ...rest,
  ...
  try {
    ...
    if (!baseRecord.id_telegram) {
      ...
      if (alreadyHasPos1.length > 1) {
        return res.status(400).json({ error: 'В базе уже есть несколько записей с position=1 для этой квартиры! Обратитесь к администратору.' });
      }
...
routerNearby.get('/api/nearby/:id/info', async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query('SELECT info FROM houses WHERE id = $1', [id]);
    if (rows.length === 0) {
...
routerNearby.post('/api/nearby/unlink', async (req, res) => {
  ...
  try {
    ...
    // Найти строку по id
    const { rows } = await pool.query('SELECT * FROM houses WHERE id = $1', [id]);
    if (!rows.length) {
    ...
    if (row.position === 1) {
      ...
      console.log('UNLINK: найдено строк по адресу:', allRows.length, allRows.map(r => r.id));
      if (allRows.length === 1) {
        await pool.query('UPDATE houses SET id_telegram = NULL WHERE id = $1', [id]);
        ...
        await pool.query(
          'UPDATE houses SET id_telegram = $1 WHERE id = $2',
          [next.id_telegram, id]
        );
        console.log(`UNLINK: переносим id_telegram (${next.id_telegram}) из строки ${next.id} в строку ${id}`);
        await pool.query('DELETE FROM houses WHERE id = $1', [next.id]);
        console.log('UNLINK: удалена строка', next.id);
        return res.json({ message: 'position 1 replaced with position 2, position 2 deleted' });
      }
    ...
    res.status(500).json({ error: 'Error unlinking id_telegram', details: error.message });
  }
});
